...
def preprocess(dt):
    ...
    mlb = MultiLabelBinarizer()
    d = DataFrame(mlb.fit_transform(x_tr.email.values), x_tr.index, mlb.classes_)
    x_tr = x_tr.drop('email', 1).join(d)
    mlb = MultiLabelBinarizer()
    d = DataFrame(mlb.fit_transform(x_ts.email.values), x_ts.index, mlb.classes_)
    x_ts = x_ts.drop('email', 1).join(d)
    return dt, x_tr,x_ts,y_tr,y_ts
def sigmoid(z):
    return 1.0 / (1.0 + exp(-z))
def sigmoid_derivatv(z):
    return sigmoid(z) * (1 - sigmoid(z))
def softmax(z):
    return exp(z) / float(sum(exp(z)))
def weit_initiat(inpt_nurn,hddn_nurn_1,hddn_nurn_2,otpt_nurn):
    net = []
    w1 = [{"weit" : [random.randn(inpt_nurn, hddn_nurn_1)]}]
    net.append(w1)
    w2 = [{"weit" : [random.randn(hddn_nurn_1, hddn_nurn_2)]}]
    net.append(w2)
    w3 = [{"weit" : [random.randn(hddn_nurn_2, otpt_nurn)]}]
    net.append(w3)
    return net
def forward(net, row):
	inputs = row
...
def backward(net, pred, row, lrnin_rate):
	for i in reversed(range(len(net))):
		...
		for j in range(len(layer)):
			nurn = layer[j]
			nurn['del'] = errs[j] * sigmoid_derivatv(nurn['val'])
